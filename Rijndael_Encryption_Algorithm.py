...
def addroundkey(k, m):
    ...
    b = [k[8]+k[9], k[10]+k[11], k[12]+k[13], k[14]+k[15]]
    c = [k[16]+k[17], k[18]+k[19], k[20]+k[21], k[22]+k[23]]
    d = [k[24]+k[25], k[26]+k[27], k[28]+k[29], k[30]+k[31]]
    w.append(a)
    w.append(b)
    w.append(c)
    w.append(d)
    ...
    x = []
    e = [m[0]+m[1], m[2]+m[3], m[4]+m[5], m[6]+m[7]]
    f = [m[8]+m[9], m[10]+m[11], m[12]+m[13], m[14]+m[15]]
    g = [m[16]+m[17], m[18]+m[19], m[20]+m[21], m[22]+m[23]]
    h = [m[24]+m[25], m[26]+m[27], m[28]+m[29], m[30]+m[31]]
    x.append(e)
    x.append(f)
    x.append(g)
    x.append(h)
...
def sboxmatrix(m):
    ...
    b = [m[8]+m[9], m[10]+m[11], m[12]+m[13], m[14]+m[15]]
    c = [m[16]+m[17], m[18]+m[19], m[20]+m[21], m[22]+m[23]]
    d = [m[24]+m[25], m[26]+m[27], m[28]+m[29], m[30]+m[31]]
    w.append(sbox(a))
    w.append(sbox(b))
    w.append(sbox(c))
    w.append(sbox(d))
...
def main():
    ...
    if(len(ptext) == 16 and len(ktext) == 16):
        ...
        print(m[1])
       # for x in range(1, 11):
        #    print("Encipher Round [%d]" % (x))
         #   m.append(sboxmatrix(m[x]))
          #  print(m[x+1]) #Last one is 10
         #   m.append(addroundkey(k[x], m[x]))
          #  print(m[x]) #Last one is 10
    else:
